...
/**
 * A declarative module that does not merge added chunks to existing chunks and
 * uses a {@link CollectionSearchSystem}.
 *
 */
public class NonMergingDeclarativeModule6 extends DefaultDeclarativeModule6 implements INonMergingDeclarativeModule {

	private static final transient Log LOGGER = LogFactory.getLog(NonMergingDeclarativeModule6.class);
	...
	public CompletableFuture<IChunk> addChunk(final IChunk chunk) {
		...
		if ((getBusyChunk() == null && name.equals("busy")) || (getEmptyChunk() == null && name.equals("empty"))
				|| (getErrorChunk() == null && name.equals("error")) || (getFreeChunk() == null && name.equals("free"))
				|| (getFullChunk() == null && name.equals("full")) || (getNewChunk() == null && name.equals("new"))
